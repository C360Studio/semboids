...
- [ ] 1.1 Failing tests for a `drainPool` (semaphore + `sync.WaitGroup`):
...
- [ ] 1.2 Implement `drainPool` (`newDrainPool(n)`, `submit(ctx, func())`,
      `wait()`); n clamps to ≥ 1.

## 2. Wire create + cull drains through the pool (`internal/sim`) — TDD

- [ ] 2.1 Failing tests: `createPending` submits each drained ID to the pool
...
- [ ] 2.2 Add `lifecycle_drain_concurrency` to the sim `Config` (+ ConfigSchema,
...
- [ ] 3.1 `task check:push` green (lint, `-race` unit + integration — the
      existing `cull_integration_test.go` full chain still passes under the
      concurrent drain).
- [ ] 3.2 Re-run the churn campaign at `lifecycle_drain_concurrency ∈ {1, 8}`
...
- [ ] 3.3 `openspec validate parallel-lifecycle-drain --strict`; refresh the
