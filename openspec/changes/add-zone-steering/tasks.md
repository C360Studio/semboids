...
- [ ] 1.1 Spike: message-path rule condition addressing — integration test
      ...
      design.md
- [ ] 1.2 Spike: rule hot-reload surface — locate the component config-update
      ...
      SemStreams issue and switch task 5.3 to the app-side gate fallback (D5)

## 2. zone-model (`internal/zone`) — TDD

- [ ] 2.1 Failing tests: config validation (unknown type, radius <= 0,
      ...
      strength triples)
- [ ] 2.2 Implement Zone struct + validation, Graphable payload, vocabulary,
      payload-registry registration
- [ ] 2.3 Boot ingestion: ensure ENTITY stream from config, publish zones
...
- [ ] 3.1 Failing tests (`internal/flock`): external steering term summed
...
- [ ] 3.2 Implement engine external-steering hook (per-boid vector slice
      staged before Tick; no locks/channels/I-O inside the per-boid loop)
- [ ] 3.3 Failing tests (`internal/sim`): edge-triggered containment (one
...
- [ ] 3.4 Implement sim: containment tracking + event publishing
      (`boids.zone.events`, `zone.transition.v1` payload), `boids.steering`
      subscription + mutex-staged table drained per tick, frame extension
      (zones array + 6th tuple element `m`)

## 4. Rules + flow wiring

- [ ] 4.1 `configs/rules/zone-steering/`: predator-flee, food-attract,
      wind-bias rules (conditions per spike 1.1; publish actions with
      `$message.*` forwarding; exit → `cancel`), `enabled: true` defaults
- [ ] 4.2 Wire the flow: register `graph-ingest` + `processor/rule` in
